...
- **Spike (both readers' full operational + security ledger, symmetric):** GHCR read-by-digest ergonomics, child-manifest cleanup, cleanup-credential capability, cold-read fan-out vs rate limits; Releases per-asset ceiling vs body cap + 1000-asset behavior; the ~2 GB path (ROBUST-02); cross-OS round-trip (Decision 6). (Resolved on paper, off the spike: GHCR atomic create-if-absent is unavailable — best-effort no-overwrite, low-severity; the Nx PUT floor is a hard `200`/Nx-21+; the write-trust backstop is host-detected fail-closed.)
