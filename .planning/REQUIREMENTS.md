...
- [ ] **TEST-07**: conformance fixture that **hashes the full vendored Nx spec** and **pins a named Nx version** (not `info.version`), asserting the server's success/401/403/404/409 + required `Content-Length`; declared floor = Nx 21 (server returns `200`)
...
### Trust & CREEP-safety
- [ ] **TRUST-01**: write-trust = **allowlist** (configured replaces default; else default implicit); default-deny; **no denylist**. `pull_request`/`release` are in the default allowlist **only when GitHub's server-side read-only-token backstop is detected** (default OFF otherwise, e.g. GHES below the floor). The in-code gate is defense-in-depth only
...
- [ ] **TRUST-06**: an **installable PPE-hygiene gate** (reusable workflow / composite action, shipped) asserting no workflow runs untrusted code in default-branch context; **default-branch protection** stated as an adopter prerequisite; heuristic linters are defense-in-depth
- [ ] **TRUST-07**: first-write-wins/no-overwrite (409) per adapter — CREEP value **conditional on TRUST-02**; Actions cache native; **GHCR requires an atomic create-if-absent (GO/NO-GO); absent it, no-overwrite is best-effort and TRUST-02 is the containment** (GHCR-conditional)
- [ ] **TRUST-08**: the mirror publishes **only server-produced keys** (distinguishing namespace/prefix), never "any 1-512 hex" Actions-cache key
...
- [ ] **RETAIN-03**: cleanup credential = job-scoped `GITHUB_TOKEN` where sufficient; org-owned needs a classic PAT (`delete:packages`) **behind an Actions Environment with required reviewers** (or keep GHCR in-repo); never in a PR-triggered workflow; deletion via first-party Octokit; `concurrency:` group (queue, don't cancel); GHCR child-manifest deletion is reference-checked/fail-closed and the reader degrades a missing child to MISS
...
*Last updated: 2026-07-17 - applied the 6-panel triage survivors (spike/PoC framing; sync gate {push,schedule}; capability-gated write-trust; shipped PPE gate; cross-OS CORR-01; observability; governance; split docs; per-primitive ceilings; GHCR/Releases-conditional spike gates). See ARCHITECTURE-DECISION.md.*
