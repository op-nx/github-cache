...
### Trust & CREEP-safety
- [ ] **TRUST-01**: write-trust = **allowlist** (configured replaces default; else default implicit); default-deny; **no denylist**. `pull_request`/`release` are in the default allowlist **only when GitHub's server-side read-only-token backstop is present, via a runtime-derived, fail-closed check** (github.com/Data-Residency vs GHES-below-floor) — **default OFF on any uncertainty, never a caller-supplied flag**. The in-code gate is fork-spoofable defense-in-depth only, so a fail-open detection would leave no real control
...
*Last updated: 2026-07-17 - applied the 6-panel triage survivors, then an independent Sonnet `/lz-security-review`: TRUST-01 fail-closed runtime-derived detection; TRUST-06 PPE gate relabeled advisory; TRUST-08 sequenced before private mirror; added TRUST-09 (publish-time visibility assert); RETAIN-03 prefers in-repo GHCR. See ARCHITECTURE-DECISION.md.*
