...
| Correctness | Cross-OS cache isolation (CORR-01) rests on INCIDENTAL mechanisms for the Actions-cache backend, not a declared one. Measured in quick 260725-rk4 (probe run 30173654069): a windows-11-arm `build` MISSED and wrote a DIFFERENT key (nx-cache-13655686526929222562) than ubuntu's (nx-cache-14522047022641658505), so the two OSes diverge in the Nx TASK HASH even though only `integration` declares `{"runtime":"node -p process.platform"}`. The keys never collided, so the `@actions/cache` version-hash-over-`join(tmpdir(),...)` layer that PROJECT.md cites as the protection is UNTESTED. Whatever makes build/typecheck/test diverge is a side effect (project-configuration divergence), and `integration` needing a hand-added discriminator proves such divergence is not universal -- so an Nx change that normalized it would silently permit cross-OS restores of wrong-OS artifacts (a direct Core Value violation). Fix direction: declare the discriminator on every cached target, or make OS explicit in the cache key, plus a probe that forces two OSes onto one key. | later milestone (maintainer decision, 2026-07-25) | 2026-07-25 |
