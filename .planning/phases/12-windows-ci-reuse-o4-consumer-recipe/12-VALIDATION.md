...
| The detector goes green on a real `windows-11-arm` runner | XOS-05 | `workflow_dispatch` only fires for workflow files present on the DEFAULT branch, so a new file cannot be dispatched pre-merge | **CLOSED.** Run `30603713356`, `event: workflow_dispatch`, job `detect` success, runner label `windows-11-arm` (confirmed via the jobs REST payload). The gate is the printed LINE and it is genuine Nx output, not the echoed `grep -q` line that also carries it -- both occurrences located, one at the command echo and one at ` NX   Successfully ran targets build, typecheck, test for project @op-nx/github-cache`. Non-vacuity holds: **0** `[remote cache]` markers in the whole log, so all three targets EXECUTED. The pre-merge block was cleared by an operator-approved temporary `main` placement with a verified backup-and-restore, so the documented constraint is CONFIRMED rather than circumvented. **SUPERSEDED by quick 260803-mew:** the evidence above proves the THREE-target needle at `e757d4c`. `9e79009` replaced the needle with the FOUR-target `Successfully ran targets build, typecheck, test, lint for project`, and `git merge-base --is-ancestor 9e79009 e757d4c` is FALSE -- run `30603713356` predates the current needle and cannot close it. The four-target form is closed in BOTH directions by run **`30825110047`** (PASS, `event: workflow_dispatch`, `headSha 41f65e1`, needle as genuine Nx output, `lint` observed executing on `windows-11-arm`) and run **`30825602626`** (FAIL on a throwaway 3-of-4 tree: step 5 red while the plural three-target Nx line proves `nx` exited 0 in that same step, and the four-target needle occurs exactly ONCE and only as the echoed command). Note this row's "non-vacuity holds" argument rested on `[remote cache]` counts, which is a DIFFERENT non-vacuity question from whether the gate can fail at all -- the latter had never been observed on a runner and now is. See `260803-mew-EVIDENCE.md` |
