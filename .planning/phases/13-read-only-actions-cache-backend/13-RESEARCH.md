...
|---|---|---|---|
| A1 | The Nx client tolerates a PUT 403 without failing the build or producing alarming output | Q5 / Code Examples | LOW. The two existing read-only backends already 403 every write and `docs/advanced.md:28` documents it as silent. The blast radius is log noise, addressed by a comment, not by code. **CONFIRMED 2026-08-03 by quick `260803-0rr`** -- NOT on the landing run, which could not settle it (every task HIT, so no PUT was attempted). Answered instead by local measurement: four PUTs across two runs, each refused 403, Nx silent on all nine noise tokens, build green. `13-EVIDENCE.md` ADDENDUM 3 |
| A2 | `.planning/**` is in no target's declared Nx input set, so a `.planning`-only PR rotates no hash | Q4 observation | MEDIUM for the observation's design only. If wrong, the Case-B PR silently becomes Case A and proves nothing. **HELD -- verified before the Case-B PR was opened** (quick `260802-toz`; the measurement is in `13-01-SUMMARY.md`) and then confirmed by run `30768540898` itself, where no hash rotated and every producer HIT. Nothing left to check here |
...
*All three closed at planning, 2026-08-02. Each was resolved in the direction this section itself
recommended; the resolution pointer under each question names the plan and task that carries it.*
...
1. **Does the Q4 Case-B observation belong in this phase or the next?**
   ...
   - **RESOLVED as recommended.** Plan `13-01` Task 2 registers it in ROADMAP's Phase 13
     `**Live-CI close**:` block with the full procedure and the Assumption A2 verification step;
     plan `13-06` Tasks 1 and 2 both state the landing run is Case A and point at that item rather
     than restating it, so a green cannot be written up as reproducing the read half.
