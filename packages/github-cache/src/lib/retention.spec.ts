...
// retention.ts is now the single home for the nx-cache-YYYYMM tag scheme, so its
...
describe('shardTag current-month single-shard tag (D-03/D-08)', () => {
  it('is exactly nx-cache-202607 for a July 2026 date', () => {
    expect(shardTag(new Date('2026-07-19'))).toBe('nx-cache-202607');
  });

  it('zero-pads the month and reads year+month in UTC', () => {
    expect(shardTag(new Date('2026-01-05T00:00:00Z'))).toBe('nx-cache-202601');
    expect(shardTag(new Date('2026-12-31T00:00:00Z'))).toBe('nx-cache-202612');
  });
});
...
describe('isShardTag exact month-shard accepter (I8, Pitfall 4)', () => {
  ...
  // non-shard nx-cache-* tag.
  it.each(['nx-cache-202607', 'nx-cache-202601', 'nx-cache-202612'])(
    'accepts the genuine month shard %s',
    (tag) => {
      expect(isShardTag(tag)).toBe(true);
    },
  );

  // EVERY FIXTURE BUT THE LAST CARRIES THE CURRENT PREFIX, and that is the whole point of
  // the block rather than an incidental spelling. Each one exists to be rejected on the
  // 6-digit SUFFIX check; a fixture spelled with a STALE prefix is rejected on the prefix
  // instead, so the assertion stays green while testing nothing at all -- the \d{6}
  // exactness that keeps `-latest` / `-backup` out of the cleanup scope would be entirely
  // uncovered by a fully green suite. When the prefix moves, these move with it.
  //
  // `v1.0.0` is the deliberate exception: it is the FOREIGN-tag case, and a prefix mismatch
  // is exactly the rejection it exists to assert.
  it.each([
    'nx-cache-',
    'nx-cache-2026',
    'nx-cache-20260',
  ...
  it('derives from the single-sourced SHARD_TAG_PREFIX (one home for the scheme)', () => {
    ...
    expect(SHARD_TAG_PREFIX).toBe('nx-cache-');
    expect(SHARD_TAG_PATTERN.test('nx-cache-202607')).toBe(true);
    expect(SHARD_TAG_PATTERN.test('nx-cache-latest')).toBe(false);
  });
});
